    ...
    <table className="w-full text-sm">
      <thead className="bg-gradient-to-r from-[#022758] to-[#1a3a5c] text-white">
        <tr>
          ...
          <th className="px-3 py-3 text-left text-xs font-bold uppercase">Model/Type</th>
          ...
          <th className="px-3 py-3 text-left text-xs font-bold uppercase">Available</th>
          <th className="px-3 py-3 text-left text-xs font-bold uppercase">Min</th>
          <th className="px-3 py-3 text-left text-xs font-bold uppercase">Status</th>
          <th className="px-3 py-3 text-left text-xs font-bold uppercase">Updated</th>
          <th className="px-3 py-3 text-left text-xs font-bold uppercase">Action</th>
        </tr>
      </thead>
      <tbody>
        {controller.pageRows.length === 0 ? (
          <tr>
            <td colSpan={9} className="px-3 py-8 text-center text-slate-500">No stock rows found.</td>
          </tr>
        ) : (
          controller.pageRows.map((row) => (
            <tr key={row.id} className="border-b border-slate-100 hover:bg-blue-50/40">
              ...
              <td className="px-3 py-2 text-xs text-slate-600">{row.modelOrType}</td>
              <td className="px-3 py-2 text-xs text-slate-600">{row.size}</td>
              <td className="px-3 py-2 text-xs text-slate-600">{row.moc}</td>
              <td className="px-3 py-2 text-xs font-semibold text-slate-900">{controller.formatNumber(row.available)}</td>
              <td className="px-3 py-2 text-xs text-slate-600">{controller.formatNumber(row.minStockLevel)}</td>
              <td className="px-3 py-2 text-xs">
                <span className={`rounded-md px-2 py-0.5 text-[10px] font-semibold ${controller.STOCK_STATUS_COLORS[row.health]}`}>
                  {row.health.toUpperCase()}
                </span>
              </td>
              <td className="px-3 py-2 text-xs text-slate-600">{controller.formatDate(row.updatedAt)}</td>
              <td className="px-3 py-2">
...
export default function StockManagementPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50">
      <header className="sticky top-0 z-40 bg-[#022758]">
      ...
      <main className="mx-auto max-w-7xl px-3 py-4 sm:px-4 lg:px-6">
        ...
        <div id="filterRow" className="mb-3 flex flex-wrap items-center gap-2 md:flex-nowrap">
          ...
          <div className="min-w-0 flex-1">
            <div className="relative">
              <input
                ...
                placeholder="Search by item, model, size, MOC..."
                className="w-full rounded-lg border border-slate-300 bg-white py-2 pl-9 pr-3 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-md bg-gradient-to-r from-[#022758] to-[#1a3a5c] px-3 py-2 text-sm font-semibold text-white shadow"
          >
        ...
        <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-lg">
          {controller.error ? (
            <div className="px-4 py-3 text-sm font-medium text-red-600">{controller.error}</div>
          ) : null}
                ...
                <div key={row.id || `${row.poNo}-${index}`} className="rounded-lg border border-slate-200 bg-gradient-to-br from-white to-slate-50 p-3 shadow-sm">
                  ...
                  {controller.activeTab === 'po' ? (
                    ...
                    <p className="text-xs text-slate-600">
                      {row.modelOrType} • {row.size} • Avl {controller.formatNumber(row.available)}
                    </p>
                  )}
          ...
          <div className="flex items-center justify-between border-t border-slate-200 bg-slate-50 px-3 py-2.5 sm:px-4">
